...
def cast_columns_to_types(df, type_mapping):
    ...
    for col, target_dtype in type_mapping.items():
        ...
        try:
            ...
            print(f"✗ {col}: Conversion failed - {error}")
            raise
...
def convert_string_dates_to_datetime(df, date_columns, date_format=None):
    """Convert string columns to datetime, preferably with an explicit format."""
    df_typed = df.copy()
...
def convert_currency_to_float(df, currency_columns):
    """Strip currency symbols and convert currency columns to floats."""
    df_typed = df.copy()
    ...
    for col in currency_columns:
        ...
        try:
            ...
            if failed_conversions > 0:
                print(
                    f"⚠ {col}: {failed_conversions} values could not be "
...
def convert_integers_to_boolean(df, boolean_columns):
    """Convert 0/1 or yes/no columns to proper boolean types."""
    df_typed = df.copy()
...
def compare_dtypes(df_original, df_typed):
    """Compare dtypes before and after conversion and save a report."""
